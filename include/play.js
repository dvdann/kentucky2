  ...
  let thumb;
    ...
    try {
      const newsong = new MessageEmbed()
        .setDescription(`Started playing: [${song.title}](${song.url}) [<@!${message.author.id}>]`)
        .setColor("RANDOM")
        //.setTitle(song.title)
        //.setURL(song.url)
        //.setColor("RANDOM")
    ...
    collector.on("collect", async (reaction, user) => {
      ...
      switch (reaction.emoji.id) {
        ...
        case "769915194444480542":
          ...
          const skipembed = new MessageEmbed().setColor("RANDOM").setDescription(`${user.username} skipped the song.`)
        ...
        case "769938447279456296":
          ...
          .setDescription("Searching...").setFooter("Lyrics")
          .setColor("RANDOM")
        ...
        case "769912238236106793":
          ...
          if (queue.playing) {
            ...
            const pausemebed = new MessageEmbed().setColor("RANDOM")
              .setDescription(`${user.username} paused the music.`, "https://cdn.discordapp.com/emojis/769912238236106793.png")
            queue.textChannel.send(pausemebed).catch(console.error);
          } else {
            queue.playing = !queue.playing;
            queue.connection.dispatcher.resume();
            const playembed = new MessageEmbed().setColor("RANDOM")
              .setDescription(`${user.username} resumed the music!`, "https://cdn.discordapp.com/emojis/769912238236106793.png")
        ...
        case "769913064194834511":
          ...
          const loopembed = new MessageEmbed().setColor("RANDOM")
            .setDescription(`Loop is now ${queue.loop ? " enabled" : " disabled"}`)
        ...
        case "769915194066862080":
          ...
          if (!canModifyQueue(member)) return;
          queue.songs = [];
          const stopembed = new MessageEmbed().setColor("RANDOM").setDescription(`${user.username} stopped the music!`)
